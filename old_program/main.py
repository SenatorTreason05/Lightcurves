...
from old_program.lightcurve_generator import get_light_data, parse_light_data, plot
...
# input("Use inline svg? Not recommended for a large quantity of sources")
...
def print_data_product_progress():
    ...
    while True:
        ...
        data_products_count = len(glob(path.join(download_directory, "**/*.gz"), recursive=True))
        print(
...
for count, source_directory in enumerate(source_directories):
    ...
    for observation_directory in os.listdir(path.join(DATA_DIRECTORY, source_directory)):
        print(f"Processing observation {observation_directory}...")
        observation_full_path = path.join(DATA_DIRECTORY, source_directory, observation_directory)
        ...
        light_curve_data, observation_properties = parse_light_data(observation_full_path)
        path_to_plot_image = plot(light_curve_data, observation_full_path, bin_time)
        with open(file=path_to_plot_image, mode="r", encoding="utf-8") as svg_file:
            plot_image = svg_file.read()
        render_data[source_directory].append((observation_properties, plot_image))
